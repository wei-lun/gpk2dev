...
// 初始化變數
// 功能標題，放在標題列及meta
$function_title =  $tr['membercenter_member_changepwd'];
...
if (isset($_SESSION['member']) AND $_SESSION['member']->therole != 'T') {
	...
	if ($m[0] != 1) {
		...
		// $tr['update login password'] = '修改登入密碼';
		$member_account_col_name = $tr['change login password'];
		if($config['site_style']=='mobile'){
		$member_account_col = $member_account_col . '
		<tr class="row">
			<th>' . $tr['current password'] . '</th>
			<td>
			<i class="fas fa-pencil-alt pen_icon"></i>
				<input type="password" class="form-control" id="current_password" placeholder="*' . $tr['current password'] . '">
			</td>
		</tr>
		<tr class="row">
			<th>' . $tr['modify password'] . '</th>
			<td>
				<i class="fas fa-pencil-alt pen_icon"></i>
				<input type="password" class="form-control" id="change_password_valid1" placeholder="*' . $tr['expect password'] . '">
			</td>
		</tr>
		<tr class=" row">
			<th>' . $tr['password verify'] . '</th>
			<td>
			<i class="fas fa-pencil-alt pen_icon"></i>
				<input type="password" class="form-control" id="change_password_valid2" placeholder="*' . $tr['password verify again'] . '">
			</td>
		</tr>
		';
		}else{
			$member_account_col = $member_account_col . '
			<tr class="row">
				<th class="col-2">' . $tr['current password'] . '</th>
				<td class="col">
					<input type="password" class="form-control" id="current_password" placeholder="*' . $tr['current password'] . '">
				</td>
			</tr>
			<tr class="row">
				<th class="col-2">' . $tr['modify password'] . '</th>
				<td class="col">
					<input type="password" class="form-control" id="change_password_valid1" placeholder="*' . $tr['expect password'] . '">
				</td>
			</tr>
			<tr class=" row">
				<th class="col-2">' . $tr['password verify'] . '</th>
				<td class="col">
					<input type="password" class="form-control" id="change_password_valid2" placeholder="*' . $tr['password verify again'] . '">
				</td>
			</tr>
		...
		<button type="submit" id="submit_change_password" class="send_btn bg-primary">' . $tr['submit'] . '</button>
		';
    ...
    var change_password_valid1 = $('#change_password_valid1').val();
    var change_password_valid2 = $('#change_password_valid2').val();
    var minlength = 6;
    var maxlength = 12;

    if( change_password_valid1.length < minlength || change_password_valid1.length > maxlength ){
    			//請將底下所有 * 欄位資訊填入
    			alert('".$tr['please enter password'].'('.$tr['need to be 6 to 12'].')'."');
    }else if((current_password) == '' || (change_password_valid1) == '' || (change_password_valid2) == ''  ){
          ...
          if(change_password_valid1 == change_password_valid2 ){
            $('#submit_change_password').attr('disabled', 'disabled');
    				$.post('member_action.php?a=member_editpersondata_passwordm',
    					{
								current_password: $().crypt({method:'sha1', source:jQuery.trim(current_password) }),
    						change_password_valid1: $().crypt({method:'sha1', source:jQuery.trim(change_password_valid1) }),
								change_password_valid2: $().crypt({method:'sha1', source:jQuery.trim(change_password_valid2) }),
								csrftoken : csrftoken
    					},
    					function(result){
...
$tmpl['sidebar_content'] = ['safe','member_changepwd'];
// banner標題
