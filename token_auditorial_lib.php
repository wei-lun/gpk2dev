 ...
 * @return void
 */
function get_one_member_data($account)
{
  $member_sql = "SELECT * FROM root_member JOIN root_member_wallets ON root_member.id=root_member_wallets.id WHERE root_member.account = '".$account."' AND root_member.therole != 'R' AND root_member.status = '1';";
  $member_sql_result = runSQLall($member_sql);
...
SQL;
  ...
  $auditreport_sql_result = runSQLall($auditreport_sql);
...
function get_all_acc($member_data)
{
  ...
  foreach ($system_config['casino_list'] as $casino) {
    ...
    if (empty($member_data->$casino_acc)) {
      continue;
    }

    $casino_acc_list['casino_acc'][] = $member_data->$casino_acc;
  }
...
function get_auditorial_data($member_data)
{
  ...
  if ($withdraw_after_deposit_data != null) {
    ...
    foreach ($depositData as $k => $v) {
      ...
      $auditreport_data = (!empty(count($auditreportData)) && array_key_exists($v['id'], $auditreportData)) ? $auditreportData[$v['id']] : false;
