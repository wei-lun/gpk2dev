...
if [ -f $(dirname "$0")/shellvar ]; then
  ...
  case $1 in
    install)
      ...
      cp $SRCPATH/websiteconf_orig.php $DSTPATH/websiteconf.php
      cp $SRCPATH/cron/shellvar_orig $DSTPATH/cron/shellvar
      mv $DSTPATH/cron $CRONDSTPATH
      # cd $CRONDSTPATH
      # crontab crontab.txt
      echo '4.Installation Success!!'
      echo -e "${RED}"'Please remember to edit your config file "config.php" and "websiteconf.php"'"${reset}"
    ...
    upgrade)
      ...
      rm -rf $DSTPATH/!(tmp_jsondata|tmp_dl)
      ...
      if [ -f $BAKPATH/${NOW}/websiteconf.php ]; then
        cp $BAKPATH/${NOW}/websiteconf.php $DSTPATH/websiteconf.php
      fi
