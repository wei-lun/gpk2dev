...
if( !isset($_SESSION['member']) || ($_SESSION['member']->therole!='A') ){
	...
	$col_name =<<<HTML
	<tr class="row">
        <th scope="col-3" class="col-2">時間</th>
        <th scope="col-2" class="col-2">來源帳號</th>
        <th scope="col-3" class="col-2">反水基數</th>
		<th scope="col-4" class="col-2">代理分配比例</th>
		<th scope="col-4" class="col-2">反水金額</th>
		<th scope="col-4" class="col-2">更新時間及說明</th>
    </tr>
HTML;
	if($config['site_style']=='mobile'){
	$table_content =<<<HTML
			<div class="row filter_list_button">
				...
				<div class="col-3">
					<button id="search_send" type="button" class="btn btn-outline-secondary btn-block">搜寻</button>
				</div>
			</div>
		...
		$table_content = <<<HTML
			...
			<div class="d-flex justify-content-between align-items-center">
				<div class="translog_search">
					...
					<input id="id_q_account" type="text" name="serch_bar" class="form-control" value="" placeholder="反水傭金查詢">
					</div>
				</div>
...
HTML;
		...
		$filter_ser = <<<HTML
			...
			<button id="id_categroy_select" type="button" onclick="on_slidemenu('categroy_filter');" class="btn btn-outline-secondary dropdown-toggle" value="cate_all_val">{$tr['all category']}</button>
...
HTML;
	...
	// 桌機顯示
		...
		$indexbody_content =<<<HTML
			<div id="mo_translog" class="member_management_table">{$table_content}{$filter_ser}</div>
			<div class="summary col-12">{$summary_ammount}{$summary_table}</div>
			{$table_content_btn}
HTML;
	}
}
...
$extend_js =<<<HTML
			...
			<div class="modal-dialog modal-dialog-centered" role="document">
				<div class="modal-content modal_contentstyle">
					...
					<div class="modal-body member_management">
						<a href="moa_transaction_log.php?id=`+ id +`" class="d-block"><button type="button" class="btn btn-link btn-lg btn-block agent_transfer text-dark" value="` + id + `"><h6>{$tr['Transaction log']}</h6></button></a>
	...
	$(function(){
		...
		$("#member_list").DataTable({
			...
			"columnDefs": [
				{"targets":[0],"className":"col-2"},
				{"targets":[1],"orderable": false,"className":"col-2"},
				{"targets":[2],"className":"col-2 d-flex justify-content-center"},
				{"targets":[3],"className":"col-2 d-flex justify-content-center"},
				{"targets":[4],"orderable": false,"className":"col-2"},
				{"targets":[5],"className":"col-2"}
			],
		...
		$('#member_list tbody').on('click', 'tr', function (e) {
			e.preventDefault();
			var mdata = $("#member_list").DataTable().row(this).data(); // console.log(mdata);
